...
class ValuablePackage(Package):
    ...
    def __str__(self):
        insurance_fee = 0.05 * self.value  # Výpočet pojistného
        return super().__str__() + f", hodnota balíku: {self.value} Kč, pojistné: {insurance_fee} Kč."


    def delivery_price(self):
        # Cena přepravy z mateřské třídy
        base_price = super().delivery_price()
        # Připočtení pojistného ve výši 5 % hodnoty balíku
        insurance_fee = 0.05 * self.value
        # Celková cena přepravy včetně pojistného
